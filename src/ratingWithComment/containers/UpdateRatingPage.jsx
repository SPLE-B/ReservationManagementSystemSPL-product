...
import ModifiedFormUpdateRatingForm from '../components/ModifiedFormUpdateRatingForm'
...
const [ratingDetailData, setRatingDetailData] = useState()
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const { data: ratingDetailDataResponse } = await getRatingDetailData({ idRating  })
	...
	<Layouts.ViewContainerLayout
		buttons={
			...
			<Layouts.ViewContainerBackButtonLayout>
			  	<Link to={`/rating
			  	`}>
			  		<Button id="_BS4zoF2KEfGjCoBCLIsS7g" className="p-4" variant="secondary">
			  		  Kembali 
			  		</Button>
			  	</Link>
			  </Layouts.ViewContainerBackButtonLayout>
